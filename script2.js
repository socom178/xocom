...
/* ---- 10. FORMULAIRE CONTACT — Envoi AJAX ---- */
...
// Ajouter l'animation spin dans le head
...
if (form) {
  form.addEventListener('submit', async function (e) {
    ...
    if (!nom) {
      showToast('Veuillez entrer votre nom complet.', false);
      return;
    }
    ...
    if (!description) {
      showToast('Veuillez décrire votre projet.', false);
      return;
    }
    ...
    try {
      ...
      const response = await fetch('fichier.php', {
        method : 'POST',
        body   : formData
      });
      ...
      if (result.success) {
        ...
        const msg = result.erreurs
          ? result.erreurs.join(' | ')
          : result.message;
        showToast(msg, false);
      }
    } catch (error) {
      showToast('Erreur réseau. Contactez-nous directement sur WhatsApp.', false);
    } finally {
      setLoading(false);
    }
  });
